...
describe 'pre_command is set and user is non-root' do
  ...
  context file('/etc/passwd') do
    it { should be_file }
    its(:command) { should eq 'sudo source ~/.zshrc >/dev/null 2>&1 && sudo test -f /etc/passwd' }
  end
end
...
describe 'path pre_command and set and user is non-root' do
  ...
  context file('/etc/passwd') do
    it { should be_file }
    its(:command) { should eq 'sudo env PATH=/sbin:/usr/sbin:$PATH source ~/.zshrc >/dev/null 2>&1 && sudo env PATH=/sbin:/usr/sbin:$PATH test -f /etc/passwd' }
  end
end
